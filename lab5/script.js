...
function addRecord() {
  ...
  // Сохранение на сервер после добавления записи
  saveBudgetToServer()();
...
// Загрузка данных бюджета с сервера
async function loadBudgetFromServer() {
  try {
    const response = await fetch('http://localhost:4000/api/budget');
    const data = await response.json();
    console.log('Загруженные данные:', data); // Добавьте эту строку для проверки
...
// Сохранение данных бюджета на сервер
...
// Заменить вызовы localStorage на серверные
